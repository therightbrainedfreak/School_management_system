...
function MBlogView({blogId, setSearchParams}) {
    ...
    return (
        ...
        <div className="main-container max-sm:mx-4 border-md">
            <div onClick={() => setSearchParams({}, {replace: true})}
            ...
            <div className="flex flex-row gap-2 mb-4">
                <button onClick={()=>{navigate(`/blogs/${blogId}`, {replace: true})}} className=" flex flex-row items-center justify-center gap-1 bg-green-200 px-3 py-2 rounded-md"><MdEditNote size={"20px"}/> Edit</button>
                <button className=" flex flex-row items-center justify-center gap-1 bg-red-200 px-3 py-2 rounded-md"><MdDelete size={"16px"}/> Delete</button>
            </div>
        </div>
    )
}
